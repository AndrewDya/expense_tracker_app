...
class UI:
    ...
    def show_menu(self):
        ...
        add_button = tk.Button(self.main_frame, text="Добавить расход", command=self.add_expense)
        add_button.pack()

        view_button = tk.Button(self.main_frame, text="Просмотреть расходы", command=self.view_expenses)
        view_button.pack()

        budget_button = tk.Button(self.main_frame, text="Отслеживание бюджета", command=self.budget_tracking)
        budget_button.pack()
    ...
    def budget_tracking(self):
        ...
        BudgetTrackingView.show_budget_info(total_budget, remaining_budget)
